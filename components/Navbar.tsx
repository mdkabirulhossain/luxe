    ...
    <header className="w-full border-b border-gray-200 bg-white sticky top-0 z-50 font-sans">
      <div className="max-w-7xl mx-auto px-4 md:px-8 h-20 flex items-center justify-between gap-4">
        ...
        <nav className="hidden md:flex items-center gap-8 lg:gap-12 text-base font-medium text-black">
          <Link href="/" className={`${pathname === '/' ? 'underline underline-offset-8 decoration-2 decoration-black' : 'hover:underline underline-offset-8 decoration-1'} transition-all`}>Home</Link>
          <Link href="/products" className={`${pathname?.startsWith('/products') ? 'underline underline-offset-8 decoration-2 decoration-black' : 'hover:underline underline-offset-8 decoration-1'} transition-all`}>Product</Link>
          <Link href="/about" className={`${pathname?.startsWith('/about') ? 'underline underline-offset-8 decoration-2 decoration-black' : 'hover:underline underline-offset-8 decoration-1'} transition-all`}>About</Link>
          <Link href="/contact" className={`${pathname?.startsWith('/contact') ? 'underline underline-offset-8 decoration-2 decoration-black' : 'hover:underline underline-offset-8 decoration-1'} transition-all`}>Contact</Link>
        </nav>
        ...
        <div className="md:hidden w-full bg-white border-b border-gray-200 shadow-xl absolute left-0 top-20 z-40 transition-all duration-300 ease-in-out">
          <div className="flex flex-col p-6 space-y-4">
            ...
            <nav className="flex flex-col space-y-4 text-base font-semibold text-black">
              <Link href="/" className={`pb-2 border-b border-gray-100 ${pathname === '/' ? 'text-red-500' : 'hover:text-red-500'}`} onClick={() => setIsMobileMenuOpen(false)}>Home</Link>
              <Link href="/products" className={`pb-2 border-b border-gray-100 ${pathname?.startsWith('/products') ? 'text-red-500' : 'hover:text-red-500'}`} onClick={() => setIsMobileMenuOpen(false)}>Product</Link>
              <Link href="/about" className={`pb-2 border-b border-gray-100 ${pathname?.startsWith('/about') ? 'text-red-500' : 'hover:text-red-500'}`} onClick={() => setIsMobileMenuOpen(false)}>About</Link>
              <Link href="/contact" className={`pb-1 ${pathname?.startsWith('/contact') ? 'text-red-500' : 'hover:text-red-500'}`} onClick={() => setIsMobileMenuOpen(false)}>Contact</Link>
            </nav>
          </div>
        </div>
      )}
